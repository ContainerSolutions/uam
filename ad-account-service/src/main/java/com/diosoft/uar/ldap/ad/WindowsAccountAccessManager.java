...
//TODO move connection management inside manager?
public class WindowsAccountAccessManager implements AccessManager<WindowsAccountAccess, WindowsAccountAccessFilter> {

    private static final Logger LOG = Logger.getLogger(WindowsAccountAccessManager.class);
    ...
    public void grant(WindowsAccountAccess access) throws AccessManagerException {
        ...
        newAccountEntry.setAttribute(ATTR_LOGIN, access.getName());
        newAccountEntry.setAttribute(ATTR_UID, access.getName());
    ...
    public Collection<WindowsAccountAccess> list(WindowsAccountAccessFilter filter) throws AccessManagerException {
        ...
        try {
            Filter searchFilter = Filter.createEqualityFilter(ATTR_LOGIN, filter.getSearchString());
            ...
            if(searchResult.getEntryCount() > 0) {
                for (SearchResultEntry entry : searchResult.getSearchEntries()) {
                    WindowsAccountAccess access = new WindowsAccountAccess(
                            ...
                            entry.getAttributeValue(ATTR_MAIL)
                            );
                    result.add(access);
                }
            }
            LOG.info("Found " + searchResult.getEntryCount() + " records for " + filter.getSearchString());
        } catch (LDAPException e) {
            LOG.error("Exception occurred while searching for account [" + filter.getSearchString() + "]", e);
